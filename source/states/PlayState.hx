...
import substates.GitarooPauseSubState;
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['WHAT THE HELL?!', 0.2], //From 0% to 19%
		...
		['Peak!', 1], //From 90% to 99%
		['Peak!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'spriteRoom': new SpriteRoom(); 		// Sprite week
			case '21gamerhogWhat': new SpriteRoom2021(); // Spritebars 2021
		}
		...
		flashingShit = new FlxSprite();
		flashingShit.makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.WHITE);
		flashingShit.alpha = 0;
		flashingShit.screenCenter();

		uiGroup = new FlxSpriteGroup();
		comboGroup = new FlxSpriteGroup();
		noteGroup = new FlxTypedGroup<FlxBasic>();
		uiGroup.add(flashingShit);
		add(comboGroup);
		add(uiGroup);
		add(noteGroup);
		...
		if (ClientPrefs.data.botplayImages) {
		...
		botplaySpr = new FlxSprite(-31, 540);
		botplaySpr.frames = Paths.getSparrowAtlas('bot');
		...
		if (ClientPrefs.data.botplayImages) {
			...
			botplaySpr.visible = false;
		}
		uiGroup.add(botplaySpr);
		if (ClientPrefs.data.downScroll)
		{
			botplaySpr.y = 12;
		}

		practiceSpr = new FlxSprite(900, 540);
		practiceSpr.frames = Paths.getSparrowAtlas('pra');
		practiceSpr.scrollFactor.set();
		practiceSpr.scale.set(0.6, 0.6);
		if (ClientPrefs.data.botplayImages) {
			practiceSpr.visible = practiceMode;
		} else {
			practiceSpr.visible = false;
		}
		add(practiceSpr);
		if (ClientPrefs.data.downScroll)
		{
		...
		botplaySpr.animation.addByPrefix('idle', 'bot0');
		practiceSpr.animation.addByPrefix('idle', 'practice0');
		...
		switch(eventName) {
			...
			case 'Camera Zoom':
				if(ClientPrefs.data.camZooms)
				{
					...
					{
						defaultCamZoom = stageZoomVal;
					}
				}

			case 'Flash':
				if(ClientPrefs.data.flashing) {
					if(flValue1 == null) flValue1 = 0.2;

					var blud2:FlxColor = 0xFFFFFFFF;
					if (value2 != '' && value2 != null)
						blud2 = FlxColor.fromString(value2);

					flashingShit.alpha = 1;
					FlxTween.tween(flashingShit, {alpha: 0}, flValue1);
				}
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in 0...separatedScore.length)
		{
			...
			if(ClientPrefs.data.psychDSRatingAnims) {
	...
	override function beatHit()
	{
		...
		botplaySpr.animation.play('idle');
		practiceSpr.animation.play('idle');
