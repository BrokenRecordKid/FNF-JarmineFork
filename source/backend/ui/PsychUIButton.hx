...
class PsychUIButton extends FlxSpriteGroup
{
	...
	public var superSexyStyle = ClientPrefs.data.yooEyeColorShit;
	...
	public var clickStyleTestes:UIStyleData = {
		bgColor: FlxColor.BLUE,
		textColor: FlxColor.WHITE,
		bgAlpha: 1
	};
	public var hoverStyleTestes:UIStyleData = {
		bgColor: FlxColor.WHITE,
		textColor: FlxColor.BLUE,
		bgAlpha: 1
	};
	public var normalStyleTestes:UIStyleData = {
		bgColor: 0xFFAAAAAA,
		textColor: FlxColor.BLUE,
		bgAlpha: 1
	};
	...
	override function update(elapsed:Float)
	{
		...
		{
			// Testes
			if (superSexyStyle == "Blue") {
				bg.color = normalStyleTestes.bgColor;
				bg.alpha = normalStyleTestes.bgAlpha;
				text.color = normalStyleTestes.textColor;
				_firstFrame = false;
			} else {
		...
		if(forceCheckNext || FlxG.mouse.justMoved || FlxG.mouse.justPressed)
		{
			...
			if(!isClicked)
			{
				var style:UIStyleData = (overlapped) ? hoverStyleTestes : normalStyleTestes;
			...
			if(overlapped && FlxG.mouse.justPressed)
			{
				isClicked = true;
				if (superSexyStyle == "Blue") {
					bg.color = clickStyleTestes.bgColor;
					bg.alpha = clickStyleTestes.bgAlpha;
					text.color = clickStyleTestes.textColor;
				} else {
